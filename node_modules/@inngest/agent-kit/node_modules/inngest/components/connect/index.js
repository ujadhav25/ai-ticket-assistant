...
const WorkerHeartbeatInterval = 10000;
...
class WebSocketWorkerConnection {
    constructor(options) {
         ...
         */
        this.inProgressRequests = new waitgroup_1.WaitGroup();
        /**
    ...
    async sendStartRequest(hashedSigningKey, attempt) {
        ...
        if (envOverride) {
            headers[consts_js_1.headerKeys.Environment] = envOverride;
        }
        ...
        if (!resp.ok) {
            if (resp.status === 401) {
                throw new util_js_1.AuthError(`Failed initial API handshake request to ${targetUrl.toString()}${envOverride ? ` (env: ${envOverride})` : ""}, ${await resp.text()}`, attempt);
            }
            if (resp.status === 429) {
    ...
    async prepareConnection(requestHandlers, hashedSigningKey, data, attempt, path = []) {
        const connectionId = (0, ulidx_1.ulid)();
        ...
        const heartbeatInterval = setInterval(() => {
            ...
            setTimeout(() => {
                if (!conn.lastGatewayHeartbeatAt) {
                    ...
                    void onConnectionError(new util_js_1.ReconnectError(`Gateway heartbeat missed (${connectionId})`, attempt));
                    return;
                }
                const timeSinceLastHeartbeat = new Date().getTime() - conn.lastGatewayHeartbeatAt.getTime();
